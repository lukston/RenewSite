...
# --- Upload GeoTIFF ---
uploaded_tif = st.file_uploader("Upload a GeoTIFF file (e.g., wind speed base)", type=["tif", "tiff"])
if not uploaded_tif:
    st.info("Upload a GeoTIFF file to simulate score overlay (0–100)")
    st.stop()

# --- Erzeuge neues Score-TIFF auf Basis der Upload-Datei ---
@st.cache_resource
def generate_score_tif(uploaded_tif):
    with NamedTemporaryFile(delete=False, suffix=".tif") as tmp_in:
        tmp_in.write(uploaded_tif.read())
...
tif_path = generate_score_tif(uploaded_tif)
...
def load_score_data(tif_path):
    ...
    return score_array, out_transform, raster_bounds, germany

score_data, transform, bounds, germany = load_score_data(tif_path)

# --- Create overlay image with boosted contrast ---
@st.cache_resource
