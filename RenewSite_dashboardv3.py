...
map_tab, finance_tab, site_tab,  table_tab, lcoe_tab, news_tab = st.tabs([
    "🌍 Wind Map",
    "📊 Financial Dashboard",
    "📍 Site Dashboard",
    "📄 Site Score Table",
...
with site_tab:
    ...
    if not st.session_state.get("st_data", {}).get("last_active_drawing"):
        ...
        with col1:
            ...
            st.markdown(f"<span style='font-size:2.4em; font-weight:bold'>{coal_offset:,.0f} t/y</span>",
                        unsafe_allow_html=True)
        with col2:
            st.markdown("**Annual CO₂ offset vs Gas**")
            st.markdown(f"<span style='font-size:2.4em; font-weight:bold'>{gas_offset:,.0f} t/y</span>",
                        unsafe_allow_html=True)
        ...
        trees = coal_offset / 0.016  # slow-growing tree

        col1, col2, col3 = st.columns(3)
        with col1:
            st.markdown("<div style='font-size:3em; text-align:center;'>🚗</div>", unsafe_allow_html=True)
            st.markdown(f"<div style='font-size:2em; font-weight:bold; text-align:center'>{int(cars):,}</div>",
                        unsafe_allow_html=True)
        ...
        with col2:
            ...
            st.markdown(f"<div style='font-size:2em; font-weight:bold; text-align:center'>{int(flights):,}</div>",
                        unsafe_allow_html=True)
            st.markdown("<div style='text-align:center;'>Transatlantic flights avoided</div>", unsafe_allow_html=True)
        with col3:
            st.markdown("<div style='font-size:3em; text-align:center;'>🌳</div>", unsafe_allow_html=True)
            st.markdown(f"<div style='font-size:2em; font-weight:bold; text-align:center'>{int(trees):,}</div>",
                        unsafe_allow_html=True)
...
with finance_tab:
    if not st.session_state.get("st_data", {}).get("last_active_drawing"):
        ...
        availability     = 0.95
        lifetime = 35
        ramp_up          = [0.75] + [1.0]*(lifetime-1)
        degradation      = [(1 - 0.005*t) for t in range(lifetime)]
        ...
        for t in range(lifetime):
            yr = str(2024 + t)
            if yr in price_scenarios_df.columns:
                base_price = price_scenarios_df.at[scenario, yr]
            else:
                base_price = 0.0
            prices[t] = base_price * ((1 + inflation_rate / 100) ** t)  # always inflation adjusted

        depreciation = capex_eur / lifetime
        cost_of_equity_rate = cost_of_equity / 100
        interest_exp = debt_amt * (cost_of_debt/100)

        year_list, eff_MWh_list, revenue_list, opex_list = [], [], [], []
        capex_list, depreciation_list, interest_list = [], [], []
        ebit_list, ebt_list, tax_list = [], [], []
        net_income_list, debt_repay_list, net_borrow_list = [], [], []
        fcfe_list, disc_fcfe_list = [], []
        ...
        pi = (np.sum(disc_fcfe_list[1:])) / abs(fcfe_list[0]) if fcfe_list[0] != 0 else np.nan
        ...
        with col3:
            st.metric("MIRR (Equity)", f"{mirr*100:.2f}%" if not np.isnan(mirr) else "N/A")
        with col4:
            st.metric("Payback Period", payback if not np.isnan(payback) else "Never")
        with col5:
        ...
        table_df = pd.DataFrame({
            "Year": year_list,
            "Inflation-Adjusted Price (€/MWh)": prices,
    ...
    st.markdown("## Energy-Price Forecasts (2024–2058)")
    price_type_options = {
        "Nominal (not inflation-adjusted)": "Price",
        "Inflation-adjusted": "InfAdjPrice",
        "Show both": "Both"
    }
    selected_price_type = st.radio(
        "Show price as:",
        options=list(price_type_options.keys()),
        index=2  # Default: Show both
    )

    years = [str(2024 + t) for t in range(35)]
    price_df = (
    ...
    price_df["InfAdjPrice"] = price_df.apply(
        lambda row: row["Price"] * ((1 + inflation_rate / 100) ** int(row["YearIndex"])), axis=1
    )

    if price_type_options[selected_price_type] == "Both":
        plot_df = pd.melt(
            price_df,
            id_vars=["Scenario", "Year", "YearIndex"],
            value_vars=["Price", "InfAdjPrice"],
            var_name="PriceType",
            value_name="Value"
        )
        plot_df["PriceType"] = plot_df["PriceType"].map({
            "Price": "Nominal Price",
            "InfAdjPrice": "Inflation Adjusted Price"
        })
        chart = (
            alt.Chart(plot_df)
            .mark_line(point=True)
            .encode(
                ...
                color=alt.Color("Scenario:N", scale=alt.Scale(range=color_sequence), title="Scenario"),
                strokeDash=alt.StrokeDash("PriceType:N", title="Price Type")
            )
            .properties(height=350, title="Energy Prices (Nominal vs. Inflation Adjusted)")
            .configure_title(font='Arial', fontSize=22, anchor='start')
            .configure_axis(labelFont='Arial', titleFont='Arial', labelFontSize=12, titleFontSize=13)
        )
    else:
        plot_df = price_df.copy()
        col = price_type_options[selected_price_type]
        plot_df = plot_df.rename(columns={col: "Value"})
        plot_df["PriceType"] = ("Inflation Adjusted Price" if col == "InfAdjPrice" else "Nominal Price")
        chart = (
            alt.Chart(plot_df)
            .mark_line(point=True)
            .encode(
            ...
            .properties(height=350, title="Energy Prices ({})".format(plot_df["PriceType"].iloc[0]))
            .configure_title(font='Arial', fontSize=22, anchor='start')
            .configure_axis(labelFont='Arial', titleFont='Arial', labelFontSize=12, titleFontSize=13)
        )
...
with lcoe_tab:
    ...
    if not st.session_state.get("st_data", {}).get("last_active_drawing"):
        ...
        for t in range(lifetime):
            ...
            real_price = price_scenarios_df.at[scenario, y]
            price_series.append(real_price * ((1 + inflation_rate/100) ** t))
        df = pd.DataFrame({
