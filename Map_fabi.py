...
map_tab, finance_tab, price_tab, score_tab, news_tab, lcoe_tab, co2_tab = st.tabs([
    "🌍 Wind Map", "📊 Financial Dashboard",
    "⚡ Energy Prices", "📍 Site Score Map",
    "📰 News", "🧮 LCOE Calculator", "🌱 CO₂ Avoided"
])
...
with map_tab:
    ...
    m = folium.Map(
        location=[germany.geometry.centroid.y.mean(),
                  germany.geometry.centroid.x.mean()],
        zoom_start=6, tiles="cartodbpositron", width="100%"
    )
    ImageOverlay(
        image=img,
        bounds=[
    ...
    linear.viridis.scale(vmin, vmax).add_to(m)
...
# --- 4) Site Score Map ---
with score_tab:
    st.markdown("## Site Suitability (Precomputed)")
    ...
    with rasterio.open(score_path) as src:
        sd, tr = np.nan_to_num(src.read(1), nan=0., posinf=0., neginf=0.), src.transform
        ip, smin, smax = create_overlay(sd)
        smap = folium.Map(
            location=[germany.geometry.centroid.y.mean(),
                      germany.geometry.centroid.x.mean()],
            zoom_start=6, tiles="cartodbpositron", width="100%"
        ...
        ImageOverlay(
            image=ip,
            bounds=[
                [tr[5] + tr[4]*sd.shape[0], tr[2]],
                [tr[5], tr[2] + tr[0]*sd.shape[1]]
            ],
            opacity=0.6
        ).add_to(smap)
        linear.viridis.scale(smin, smax).add_to(smap)
        Draw(export=False,
             draw_options={"rectangle": {"shapeOptions": {"color": "red"}}}
        ).add_to(smap)
        st_folium(smap, height=800, use_container_width=True)
...
with co2_tab:
    ...
    if not st.session_state.get("st_data", {}).get("last_active_drawing"):
        ...
        st.markdown(f"**Annual CO₂ offset vs Coal**\n\n<span style='font-size:2.4em; font-weight:bold'>{coal_offset:,.0f} t/y</span>", unsafe_allow_html=True)
        st.markdown(f"**Annual CO₂ offset vs Gas**\n\n<span style='font-size:2.4em; font-weight:bold'>{gas_offset:,.0f} t/y</span>", unsafe_allow_html=True)
